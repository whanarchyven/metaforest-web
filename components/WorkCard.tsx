...
import { MetaforestJob, MetaforestJobTypeEnum } from "../graphql/sdk/graphql";
  ...
  return (
    <div className={"w-full h-full relative"}>
      <div
        ...
        onClick={() => {
          if (workItem.type === MetaforestJobTypeEnum.Steps) toggleWorkPop();
          else document.location.href = "/share-report";
        }}
      >
        ...
        <div className={"w-full h-full p-4 absolute top-0"}>
          <div className={"w-full h-full relative"}>
            <div className={"absolute left-0 top-0"}>
              <div
                className={
                  "w-12 h-12 green-gradient rounded-full inline-block align-top"
                }
              ></div>
              <div className={"inline-block align-top"}>
                <p className={"inline-block ml-4 font-bold w-[70%] text-xl"}>
                  {workItem.title.split(" ")[0] +
                    " " +
                    workItem.title.split(" ")[1]}
                </p>
                <p
                  className={
            ...
            <div
              className={
                "w-16 h-8 rounded-full bottom-0 right-0 absolute bg-black rounded-full flex justify-center items-center"
              }
            >
              <p className={"text-white inline-block text-xl font-medium"}>
            ...
            <div
              className={
                "w-24 h-8 rounded-full top-0 right-0 absolute bg-black rounded-full flex justify-center items-center"
              }
            >
              <p className={"text-white inline-block text-xl font-medium"}>
                {workItem.type}
