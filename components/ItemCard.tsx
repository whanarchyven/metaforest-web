...
const ItemCard = ({ item }: ItemCardInterface) => {
  ...
  return (
    <div className={'w-full h-full p-2 rounded-xl border-4 relative' + bg_color + border_color} onClick={()=>{}}>
      <div className={'w-28 h-28 relative mx-auto'}>
        ...
        <div className={'w-full h-full absolute'}>
          <img
            alt=""
            src={item?.images?.web?''+item?.images?.web:''}
          ></img>
        </div>
      </div>
      <p className={'text-center text-xl font-bold mb-4'}>{item?.itemSlot}</p>
      <div className={'grid grid-cols-2 gap-2'}>
        <div className={'grid grid-cols-1 grid-rows-3 gap-2 h-28'}>
          {increase.map((incr: any) => {
              if (incr[1]!=0&&incr[0]!='level'){
                  return (<div className={'w-full h-full rounded-full flex justify-center items-center relative'} key={incr[0]}>
                      ...
                      <div className={'w-8 h-8 rounded-full bg-white absolute left-0 p-2'}>
                          <div className={'w-full h-full relative'}>
                              ...
                              <Image
                                  alt=""
                                  src={'/images/card_generation/common/' + incr[0] + '.svg'}
                                  layout={'fill'}
                              ></Image>
                          </div>
                      </div>
      ...
      <p className={'h-5 right-2 w-20 text-right top-1 absolute text-sm'}>#{item?.idx}</p>
