...
const PopUp = ({item,togglePop,pushItemToInventory,bunny,balance,changeBalance}:ItemCardInterface) => {
    return (
        <div className={'flex flex-wrap fixed w-full h-full top-0 left-0 grey-gradient justify-center items-center'}>
            ...
            <div className={'w-8/12 flex justify-center flex-wrap relative'}>
                ...
                <div className={'relative w-4/5 mt-5 grid h-12 grid-cols-2 gap-2'}>
                    <button className={'rounded-full bg-black text-white'} onClick={()=>{if(balance>item.price){changeBalance(balance-item.price);pushItemToInventory(item);alert("Item successfully purchased!");togglePop();}else {alert("Seems like you do not have enough carrots to buy this item!")}}}>Buy</button>
